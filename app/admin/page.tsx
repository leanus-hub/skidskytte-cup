...
export default async function AdminPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return <>
    ...
    {params.error && <div className="alert error">
      <strong>Något behöver åtgärdas:</strong> {params.error === 'cup-completed' ? 'Cupen är avslutad och låst. Ändra cupens status till Pågående om den behöver öppnas för ändringar.' : params.error === 'completed-ruleset-locked' ? 'Regelverket är låst för en avslutad cup. Öppna cupen igen innan regelverket byts.' : params.error === 'ruleset-locked' ? 'Regelverket används av en avslutad cup och är låst. Skapa en ny version för nästa säsong.' : params.error}
