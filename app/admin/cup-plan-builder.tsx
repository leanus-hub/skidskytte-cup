...
export default function CupPlanBuilder({cupId,clubs}:{cupId:string;clubs:Club[]}){
 const [rows,setRows]=useState<Row[]>(Array.from({length:6},blank));
 ...
 return <form action={createPlannedRaces} className="cup-plan-builder">
  ...
  <div className="plan-toolbar"><div><strong>Planera hela cupen</strong><p className="muted">Fyll i de deltävlingar ni känner till. BiathlonTiming kopplas först när resultat finns.</p></div><span className="badge">{active} planerade</span></div>
  ...
  {rows.map((row,i)=><div className="plan-grid plan-row" key={i}>
   <strong>{i+1}</strong>
   <input aria-label={`Namn deltävling ${i+1}`} value={row.name} onChange={e=>update(i,'name',e.target.value)} placeholder={`Deltävling ${i+1}`}/>
  ...
  <div className="plan-footer"><button type="button" className="secondary-dark" onClick={()=>setRows(r=>[...r,blank()])}>+ Lägg till deltävling</button><button type="submit" disabled={!active}>Spara tävlingsplan</button></div>
 </form>;
}