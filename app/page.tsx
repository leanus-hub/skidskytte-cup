...
export default async function HomePage({ searchParams }: { searchParams: Promise<Record<string,string|undefined>> }) {
  ...
  return <>
    ...
    {visibleCups.map(cup => {
      ...
      const pointAverageRows = cupIndividuals.filter(r=>r.races_counted>0);
      ...
      const clubEfficiencyRows = cupClubs.filter(r=>r.total_starts>0);
      ...
      return <section key={cupId} className="cup-section">
        ...
        {view==='statistics' && <>
          <section className="insight-grid">
            ...
            <article className="card insight-card"><span>Högst poängsnitt</span><strong>{bestPointAverageAthletes.length===1?bestPointAverageAthletes[0].athlete_name:bestPointAverageAthletes.length>1?`${bestPointAverageAthletes.length} åkare`:'–'}</strong><small>{bestPointAverageAthletes.length?`${maxPointAverage.toFixed(2)} p/räknad start`:'Ingen statistik'}</small>{bestPointAverageAthletes.length>1&&<details className="insight-ties"><summary>Visa åkarna</summary><p>{bestPointAverageAthletes.map(r=>r.athlete_name).join(' · ')}</p></details>}</article>
            <article className="card insight-card"><span>Effektivaste klubb</span><strong>{bestClubEfficiencyClubs.length===1?bestClubEfficiencyClubs[0].club_name:bestClubEfficiencyClubs.length>1?`${bestClubEfficiencyClubs.length} klubbar`:'–'}</strong><small>{bestClubEfficiencyClubs.length?`${maxClubEfficiency.toFixed(2)} p/start`:'Ingen statistik'}</small>{bestClubEfficiencyClubs.length>1&&<details className="insight-ties"><summary>Visa klubbarna</summary><p>{bestClubEfficiencyClubs.map(r=>r.club_name).join(' · ')}</p></details>}</article>
          </section>
