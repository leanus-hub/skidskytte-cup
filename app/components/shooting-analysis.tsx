...
export default function ShootingAnalysis({rows}:{rows:Row[]}){
 ...
 const entities=useMemo(()=>{
  ...
  for(const r of analysisRows){const id=kind==='athlete'?r.athlete_id:kind==='club'?r.club_id:r.class_id;const name=kind==='athlete'?r.athlete_name:kind==='club'?r.club_name:r.class_name;const x=m.get(id)??{id,name,hits:0,shots:0,starts:new Set<string>()};x.hits+=r.shooting_hits??0;x.shots+=r.shooting_shots??0;x.starts.add(r.race_id);m.set(id,x)}
 ...
 const chartSeries=compared.map(e=>({entity:e,points:races.map((race,idx)=>{const rr=analysisRows.filter(x=>x.race_id===race.id&&(kind==='athlete'?x.athlete_id:kind==='club'?x.club_id:x.class_id)===e.id);const h=rr.reduce((s,x)=>s+(x.shooting_hits??0),0),shots=rr.reduce((s,x)=>s+(x.shooting_shots??0),0);return {idx,value:shots?pct(h,shots):null,h,shots}})}));
 ...
 return <section className="shooting-analysis">
  ...
  {compared.length>0&&<section className="card standings-card shooting-development"><h3>Utveckling över deltävlingarna</h3><p className="muted">{compared.map(e=>e.name).join(' · ')}</p>
   ...
   <div className="table-scroll"><table><thead><tr><th>Deltävling</th>{compared.map(e=><th key={e.id}>{e.name}</th>)}</tr></thead><tbody>{races.map(race=><tr key={race.id}><td><strong>{race.name}</strong></td>{compared.map(e=>{const rr=analysisRows.filter(x=>x.race_id===race.id&&(kind==='athlete'?x.athlete_id:kind==='club'?x.club_id:x.class_id)===e.id);const h=rr.reduce((s,x)=>s+(x.shooting_hits??0),0),s=rr.reduce((a,x)=>a+(x.shooting_shots??0),0);return <td key={e.id}>{s?`${pct(h,s).toFixed(1)} % (${h}/${s})`:'–'}</td>})}</tr>)}</tbody></table></div>
  </section>}
 </section>
}