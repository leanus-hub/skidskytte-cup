...
type Detail={cup_id:string;class_id:string;athlete_id:string;race_id:string;race_name:string;race_date:string|null;region_place:number|null;cup_points:number;shooting_hits:number|null;shooting_shots:number|null;is_counted:boolean};
...
export default async function AthletePage({params}:{params:Promise<{id:string}>}){
 ...
 const [{data:standing,error},{data:detail,error:detailError}]=await Promise.all([
  ...
  supabase.from('cup_result_breakdown').select('cup_id,class_id,athlete_id,race_id,race_name,race_date,region_place,cup_points,shooting_hits,shooting_shots,is_counted').eq('athlete_id',id).order('race_date',{ascending:false})
 ]);
 if(error||!standing?.length) notFound();
 if(detailError) throw new Error(`Kunde inte läsa deltävlingar för åkarprofilen: ${detailError.message}`);
 ...
 return <>
  ...
  {rows.map(row=>{const races=details.filter(d=>d.cup_id===row.cup_id&&d.class_id===row.class_id);return <section className="card standings-card" key={`${row.cup_id}-${row.class_id}`}>
   ...
   {races.length===0?<p className="muted athlete-races-empty">Inga deltävlingar kunde visas.</p>:<>
   <div className="table-scroll athlete-race-table"><table><thead><tr><th>Deltävling</th><th>Datum</th><th>Regional plac.</th><th>Poäng</th><th>Skytte</th><th>Räknas</th></tr></thead><tbody>
   {races.map(r=><tr key={r.race_id} className={r.is_counted?'':'dropped'}><td><Link className="text-link" href={`/tavlingar/${r.race_id}`}>{r.race_name}</Link></td><td>{r.race_date??'–'}</td><td>{r.region_place??'–'}</td><td><strong>{r.cup_points}</strong></td><td>{shoot(r)}</td><td>{r.is_counted?'✓ Räknas':'Struken'}</td></tr>)}
   </tbody></table></div>
   <div className="athlete-race-cards">{races.map(r=><Link href={`/tavlingar/${r.race_id}`} className={`athlete-race-card ${r.is_counted?'':'dropped'}`} key={r.race_id}><div><strong>{r.race_name}</strong><small>{r.race_date??'Datum saknas'}</small></div><div className="athlete-race-card-stats"><span><small>Plac.</small><strong>{r.region_place??'–'}</strong></span><span><small>Poäng</small><strong>{r.cup_points}</strong></span><span><small>Skytte</small><strong>{shoot(r)}</strong></span></div><span className={`badge ${r.is_counted?'':'muted-badge'}`}>{r.is_counted?'✓ Räknas':'Struken'}</span></Link>)}</div>
   </>}
  </section>})}
 </>;
}